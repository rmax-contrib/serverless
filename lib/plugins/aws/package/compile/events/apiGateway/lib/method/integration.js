...
module.exports = {
  getMethodIntegration(http, lambdaLogicalId) {
    ...
    if (type === 'AWS' || type === 'HTTP' || type === 'MOCK') {
      ...
      _.assign(integration, {
        RequestParameters: this.getIntegrationRequestParameters(http.request),
      });
    }
  ...
  getIntegrationRequestParameters(httpRequest) {
    const parameters = {};
    _.each(httpRequest.parameters, (value, key) => {
      // It is expected that 'key' is normalized to `method.request...`.
      const suffix = key.slice(7);
      parameters[`integration.${suffix}`] = key;
    });
    return parameters;
  },
