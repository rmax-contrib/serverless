...
describe('#compileMethods()', () => {
  ...
  it('should support HTTP_PROXY integration parameters', () => {
    awsCompileApigEvents.validated.events = [
      {
        functionName: 'First',
        http: {
    ...
    return awsCompileApigEvents.compileMethods().then(() => {
      expect(
        awsCompileApigEvents.serverless.service.provider.compiledCloudFormationTemplate
          .Resources.ApiGatewayMethodUsersCreatePost.Properties
          .Integration.RequestParameters['integration.request.header.foo']
      ).to.equal('method.request.header.foo');
      expect(
        awsCompileApigEvents.serverless.service.provider.compiledCloudFormationTemplate
          .Resources.ApiGatewayMethodUsersCreatePost.Properties
          .Integration.RequestParameters['integration.request.querystring.foo']
      ).to.equal('method.request.querystring.foo');
      expect(
        awsCompileApigEvents.serverless.service.provider.compiledCloudFormationTemplate
          .Resources.ApiGatewayMethodUsersCreatePost.Properties
          .Integration.RequestParameters['integration.request.path.foo']
      ).to.equal('method.request.path.foo');
    });
  });
