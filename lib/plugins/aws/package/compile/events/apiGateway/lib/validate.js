    ...
    _.forEach(this.serverless.service.functions, (functionObject, functionName) => {
      _.forEach(functionObject.events, (event) => {
        if (_.has(event, 'http')) {
          ...
          if (http.integration === 'AWS' || http.integration === 'HTTP' || http.integration === 'HTTP_PROXY') { 
            ...
            if (http.integration !== 'HTTP_PROXY') {
              ...
              if (http.response.statusCodes) {
                ...
                const responseWarningMessage = [
                  'Warning! You\'re using the HTTP_PROXY in combination with a response',
                  ` in your function "${functionName}". This is not supported and Serverless`,
                  'will remove this configuration automatically before deployment.',
                ].join('');
                this.serverless.cli.log(responseWarningMessage);
